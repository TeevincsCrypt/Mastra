...
export async function runAutomationExecution(automationId: string, callerAddress: string): Promise<ExecutionRecord> {
  ...
  recordAuditEvent({ automationId, executionId: execution.id, type: "WAYFINDER_QUOTE_REQUESTED", message: "Requesting a real Wayfinder quote." });
  ...
  const policyCheck = async (input: SwapPolicyCheckInput) => {
    ...
    recordAuditEvent({ automationId, executionId: execution.id, type: "POLICY_EVALUATION", message: "Running deterministic policy check." });
    ...
    const decision = evaluatePolicy({
      ...
      spentTodayAmount: sumSpentSince([automationId], startOfDay(now)),
      spentThisMonthAmount: sumSpentSince([automationId], startOfMonth(now)),
    });
  ...
  recordAuditEvent({ automationId, executionId: execution.id, type: "KEEPERHUB_EXECUTION_STARTED", message: "Calling KeeperHub's real execute() endpoint." });
  ...
  if (!executed.ok) {
    recordAuditEvent({ automationId, executionId: execution.id, type: "EXECUTION_FAILED", message: executed.error, metadata: { stage: executed.stage } });
    return updateExecution(execution.id, {
      status: "failed",
  ...
  const final = updateExecution(execution.id, {
  ...
  recordAuditEvent({ automationId, executionId: execution.id, type: "EXECUTION_COMPLETE", message: `Execution finished with status: ${final.status}.` });
